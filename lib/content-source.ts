...
type SanityUpdate = {
  category?: ResearchUpdate["category"]; title?: string; slug?: string; date?: string; dateLabel?: string;
...
export async function getContentPage(slug: string): Promise<ContentPage | undefined> {
  ...
  if (slug === "talks-conferences") {
    const talks = await querySanity<SanityTalk[]>(`*[_type == "talk"] | order(displayOrder asc, coalesce(date, dateLabel) desc){title,role,eventName,date,dateLabel,location,status,externalOfficialUrl,summary}`, []);
    if (!talks.length) return page;
    return { ...page, sections: talks.filter((talk) => talk.title).map((talk) => ({ number: (talk.date ?? talk.dateLabel)?.match(/\d{4}/)?.[0] ?? "—", title: talk.title!, body: talk.summary ?? "", meta: [`Role: ${talk.role ?? ""}`, `Event: ${talk.eventName ?? ""}`, `Location: ${talk.location ?? ""}`, `Dates: ${talk.dateLabel ?? displayDate(talk.date)}`, `Status: ${talk.status ?? ""}`], ...(talk.externalOfficialUrl ? { link: { label: "Official speaker listing", href: talk.externalOfficialUrl } } : {}) })) };
  }
  if (slug === "projects") {
    const projects = await querySanity<SanityProject[]>(`*[_type == "project"] | order(displayOrder asc, _updatedAt desc){title,projectType,status,summary,venueOrPublisher,relatedThemes}`, []);
    if (!projects.length) return page;
    return { ...page, sections: projects.filter((project) => project.title).map((project, index) => ({ number: String(index + 1).padStart(2, "0"), title: project.title!, body: project.summary ?? "", points: project.relatedThemes })) };
  }
  return page;
}
