# Elnaz Kyavar — Academic Website

A publication-oriented academic website built with Next.js-compatible Vinext and React.
...
Create `.env.local` from `.env.example`. Staging is non-indexed by default. Set `SITE_ENV=production` and a verified `NEXT_PUBLIC_SITE_URL` only when the canonical public domain is ready.
